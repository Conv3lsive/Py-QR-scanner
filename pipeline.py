...
def _move_clear_for_batch(
    output_folder, considered_files, found_files, move_mode="copy"
):
...
def run_action(
    ...
    if action == 0:
        from barcode_utils import file_renamer, find_barcodes
        ...
        _emit_progress(
            progress_callback, action, 1, 1, "этап", "Переименование завершено"
        )
    ...
    if action == 1:
        ...
        if not all([image_folder, csv_path, name_fields, output_folder]):
            raise ValueError(
                "Для action=1 нужны --image-folder, --csv-path, --name-fields и --output-folder"
            )
        ...
        _emit_progress(
            progress_callback, action, 1, 3, "этап", "Перемещение unsorted/noqrcode"
        )
        ...
        _emit_progress(
            progress_callback, action, 3, 3, "этап", "Распределение завершено"
        )
        return {
            "status": "ok",
    ...
    if action == 3:
        if not all([csv_path, output_folder, name_fields]):
            raise ValueError(
                "Для action=3 нужны --csv-path, --output-folder и --name-fields"
            )
...
def process_watch_batch(
    ...
    incoming_files = list(
        dict.fromkeys(
            ...
            and os.path.splitext(path)[1].lower() in WATCH_EXTENSIONS
        )
    )
    ...
    existing_hashes = (
        state.get_existing_file_hashes(file_hashes.values()) if state else set()
    )
    unique_files = []
    ...
    for path in incoming_files:
        ...
        if file_hash in existing_hashes:
            duplicates += 1
            if state:
                duplicate_hash_records.append(
                    (file_hash, "", path, "duplicate_hash", "")
                )
            continue
        ...
        for code, paths in deduped_barcodes.items():
            for path in paths:
                processed_records.append(
                    (file_hashes.get(path, ""), code, path, "processed", "")
                )
